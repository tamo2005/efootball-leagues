...
function titleRaceSeries(database: LeagueDatabase, standings: Standing[]) {
  const matchdays = Array.from(new Set(database.matches.map((match) => Number(match.matchday)).filter((day) => Number.isFinite(day) && day > 0))).sort((a, b) => a - b);
...
function TitleRaceChart({ database, standings }: { database: LeagueDatabase; standings: Standing[] }) {
  ...
  const colors = ["#a7f06e", "#6eb7f0", "#f0b35b", "#d0c0fa"];
  return <div className="title-race-chart"><div className="chart-legend">{series.map((item, index) => <span key={item.team.id}><i style={{ background: colors[index] }} />{item.team.shortName}</span>)}</div>{matchdays.length ? <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Title race points by matchday"><path d={`M 0 ${height - 12} H ${width}`} stroke="rgba(255,255,255,.14)" /><path d={`M 0 ${height / 2} H ${width}`} stroke="rgba(255,255,255,.08)" />{series.map((item, seriesIndex) => { const path = item.values.map((value, index) => `${index === 0 ? "M" : "L"} ${(index / Math.max(1, matchdays.length - 1)) * (width - 14) + 7} ${height - 12 - (value / maxValue) * (height - 24)}`).join(" "); const last = item.values.at(-1) || 0; const lastX = ((Math.max(0, item.values.length - 1) / Math.max(1, matchdays.length - 1)) * (width - 14)) + 7; const lastY = height - 12 - (last / maxValue) * (height - 24); return <g key={item.team.id}><path d={path} fill="none" stroke={colors[seriesIndex]} strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" /><circle cx={lastX} cy={lastY} r="3.5" fill={colors[seriesIndex]} /></g>; })}</svg> : <div className="chart-empty">Confirmed results will draw the title race.</div>}<div className="chart-axis">{matchdays.slice(0, 6).map((matchday) => <span key={matchday}>MD{matchday}</span>)}</div></div>;
}
