...
type View = "overview" | "fixtures" | "teams" | "table" | "analytics" | "chronicle" | "database" | "rules";
...
function MatchRow({ database, match, user, isAdmin, onResult, onConfirm, onReschedule, onSelect }: { database: LeagueDatabase; match: Match; user: UserAccount; isAdmin: boolean; onResult: (id: string) => void; onConfirm: (id: string) => void; onReschedule: RescheduleHandler; onSelect?: (id: string) => void }) {
  ...
  const mvp = Object.entries(match.goals.reduce<Record<string, number>>((counts, goal) => { counts[goal.playerName] = (counts[goal.playerName] || 0) + 1; return counts; }, {})).sort((a, b) => b[1] - a[1])[0];
  ...
  return (
    <article className={`match-row ${owned ? "match-row-owned" : ""} ${settled ? "match-row-settled" : ""}`}>
      ...
      <div className="match-teams">
       ...
       {match.goals.length > 0 && <div className="match-row-intelligence"><div className="fixture-timeline-mini"><span>0′</span><div className="fixture-timeline-track">{match.goals.slice().sort((a, b) => a.minute - b.minute).map((goal, index) => <i key={`${goal.id}-${index}`} style={{ left: `${Math.max(3, Math.min(97, (goal.minute / 90) * 100))}%` }} title={`${goal.playerName} ${goal.minute}′`} />)}</div><span>90′</span></div>{mvp && <span className="match-mvp"><Trophy size={12} /> MVP signal: <strong>{mvp[0]}</strong> · {mvp[1]} goal{mvp[1] === 1 ? "" : "s"}</span>}</div>}
       <div className="match-actions">
        ...
        {onSelect && <button className="row-action row-action-secondary" onClick={() => onSelect(match.id)}><BarChart3 size={14} /> Dossier</button>}{canEnter ? <button className="row-action" onClick={() => onResult(match.id)}>{match.status === "SCHEDULED" || match.status === "POSTPONED" ? "Enter result" : "View result"}<ArrowRight size={14} /></button> : <span className="match-locked"><Clock3 size={13} />{match.status === "CONFIRMED" ? "Official" : dateOpen ? "Home team only" : `Opens ${formatMatchDate(match.date)}`}</span>}
      </div>
    </article>
  );
}

function FixtureMatchdayStepper({ database, activeMatchday, liveMatchday, onChange }: { database: LeagueDatabase; activeMatchday: number; liveMatchday: number; onChange: (matchday: number) => void }) {
  const matchdays = Array.from(new Set(database.matches.map((match) => Number(match.matchday)).filter((matchday) => Number.isFinite(matchday) && matchday > 0))).sort((a, b) => a - b);
  return <div className="matchday-stepper" role="tablist" aria-label="Matchday navigation"><button type="button" className="stepper-arrow" onClick={() => onChange(Math.max(matchdays[0] || 1, activeMatchday - 1))} aria-label="Previous matchday"><ChevronLeft size={15} /></button>{matchdays.map((matchday) => { const matches = database.matches.filter((match) => Number(match.matchday) === matchday); const complete = matches.length > 0 && matches.every((match) => match.status === "CONFIRMED"); const live = matchday === liveMatchday; return <button type="button" key={matchday} className={`matchday-step ${activeMatchday === matchday ? "selected" : ""} ${live ? "live" : ""}`} onClick={() => onChange(matchday)}><span>MD {String(matchday).padStart(2, "0")}</span><small>{complete ? "✓" : live ? "LIVE" : `${matches.length} FIX`}</small></button>; })}<button type="button" className="stepper-arrow" onClick={() => onChange(Math.min(matchdays.at(-1) || activeMatchday, activeMatchday + 1))} aria-label="Next matchday"><ChevronRight size={15} /></button></div>;
}

function FixtureTeamFilter({ teams, activeTeam, onChange }: { teams: Team[]; activeTeam: string; onChange: (teamId: string) => void }) {
  return <div className="fixture-team-filter"><button type="button" className={activeTeam === "all" ? "selected" : ""} onClick={() => onChange("all")}>All clubs</button>{teams.map((team) => <button type="button" key={team.id} className={activeTeam === team.id ? "selected" : ""} style={{ "--team-accent": team.accent } as CSSProperties} onClick={() => onChange(team.id)}><TeamMark team={team} size="sm" />{team.shortName}</button>)}</div>;
}

function projectedStandings(database: LeagueDatabase, standings: Standing[], scores: Record<string, { home: string; away: string }>) {
  const projected = new Map(standings.map((row) => [row.id, { ...row, form: [...row.form] }]));
  database.matches.filter((match) => match.status !== "CONFIRMED" && scores[match.id]).forEach((match) => {
    const homeScore = Number(scores[match.id].home);
    const awayScore = Number(scores[match.id].away);
    if (!Number.isFinite(homeScore) || !Number.isFinite(awayScore) || homeScore < 0 || awayScore < 0) return;
    const home = projected.get(match.homeTeamId);
    const away = projected.get(match.awayTeamId);
    if (!home || !away) return;
    home.played += 1; away.played += 1;
    home.goalsFor += homeScore; home.goalsAgainst += awayScore; home.goalDifference = home.goalsFor - home.goalsAgainst;
    away.goalsFor += awayScore; away.goalsAgainst += homeScore; away.goalDifference = away.goalsFor - away.goalsAgainst;
    if (homeScore > awayScore) { home.won += 1; home.points += 3; home.form = [...home.form, "W"]; away.lost += 1; away.form = [...away.form, "L"]; }
    else if (homeScore < awayScore) { away.won += 1; away.points += 3; away.form = [...away.form, "W"]; home.lost += 1; home.form = [...home.form, "L"]; }
    else { home.drawn += 1; away.drawn += 1; home.points += 1; away.points += 1; home.form = [...home.form, "D"]; away.form = [...away.form, "D"]; }
  });
  return [...projected.values()].sort((a, b) => b.points - a.points || b.goalDifference - a.goalDifference || b.goalsFor - a.goalsFor || b.won - a.won || a.name.localeCompare(b.name));
}

function StandingTable({ standings, compact = false, userTeamId }: { standings: Standing[]; compact?: boolean; userTeamId?: string }) {
  const rows = compact ? standings.slice(0, 5) : standings;
  return (
    <div className="table-wrap">
      <table className="standing-table">
        <thead><tr><th className="rank-col">#</th><th>Team</th><th>Form</th><th>P</th><th>W</th><th>D</th><th>L</th><th>GD</th><th className="points-col">Pts</th></tr></thead>
        <tbody>
          {rows.map((team, index) => (
            <tr key={team.id} className={team.id === userTeamId ? "is-user-team" : ""}>
              <td className="rank-col"><span className={`rank-number ${index === 0 ? "leader" : ""}`}>{index + 1}</span></td>
              <td><div className="standing-team"><TeamMark team={team} size="sm" /><span>{team.name}</span>{team.id === userTeamId && <em>YOU</em>}</div></td>
              <td><div className="standing-form">{team.form.slice(-5).map((value, formIndex) => <FormPill key={`${team.id}-${formIndex}`} value={value} />)}</div></td>
...
function MiniSparkline({ values, tone = "#a7f06e" }: { values: number[]; tone?: string }) {
  ...
  const points = values.map((value, index) => `${(index / Math.max(1, values.length - 1)) * width},${height - ((value - min) / span) * (height - 5) - 2}`).join(" ");
  return <svg className="mini-sparkline" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Form trend"><polyline points={points} fill="none" stroke={tone} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /><circle cx={Number(points.split(" ").at(-1)?.split(",")[0] || 0)} cy={Number(points.split(" ").at(-1)?.split(",")[1] || 0)} r="2.8" fill={tone} /></svg>;
}

function titleRaceSeries(database: LeagueDatabase, standings: Standing[]) {
  const matchdays = Array.from(new Set([1, ...database.matches.map((match) => Number(match.matchday)).filter((day) => Number.isFinite(day) && day > 0)])).sort((a, b) => a - b);
  const leaders = standings.slice(0, 4);
  const series = leaders.map((team) => ({
    team,
    values: matchdays.map((matchday) => database.matches
      .filter((match) => match.status === "CONFIRMED" && Number(match.matchday) <= matchday && (match.homeTeamId === team.id || match.awayTeamId === team.id))
      .reduce((points, match) => {
        if (match.homeScore === null || match.awayScore === null) return points;
        ...
        return points + (teamScore > opponentScore ? 3 : teamScore === opponentScore ? 1 : 0);
      }, 0)),
  }));
  return { matchdays, series };
}

function TitleRaceChart({ database, standings }: { database: LeagueDatabase; standings: Standing[] }) {
  const { matchdays, series } = titleRaceSeries(database, standings);
  const width = 360;
  const height = 136;
  const maxValue = Math.max(3, ...series.flatMap((item) => item.values));
  return <div className="title-race-chart"><div className="chart-legend">{series.map((item) => <span key={item.team.id}><i style={{ background: item.team.accent }} />{item.team.shortName}</span>)}</div>{matchdays.length ? <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Title race points by matchday"><path d={`M 0 ${height - 12} H ${width}`} stroke="rgba(10,10,10,.18)" /><path d={`M 0 ${height / 2} H ${width}`} stroke="rgba(10,10,10,.08)" />{series.map((item) => { const path = item.values.map((value, index) => `${index === 0 ? "M" : "L"} ${(index / Math.max(1, matchdays.length - 1)) * (width - 14) + 7} ${height - 12 - (value / maxValue) * (height - 24)}`).join(" "); const last = item.values.at(-1) || 0; const lastX = ((Math.max(0, item.values.length - 1) / Math.max(1, matchdays.length - 1)) * (width - 14)) + 7; const lastY = height - 12 - (last / maxValue) * (height - 24); return <g key={item.team.id}><path d={path} fill="none" stroke={item.team.accent} strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" /><circle cx={lastX} cy={lastY} r="3.5" fill={item.team.accent} /></g>; })}</svg> : <div className="chart-empty">Confirmed results will draw the title race.</div>}<div className="chart-axis">{matchdays.slice(0, 6).map((matchday) => <span key={matchday}>MD{matchday}</span>)}</div></div>;
}

function BroadcastHub({ database, standings, scorers, nextMatch, confirmedCount, pendingCount, liveMatchday, onFixtures, onResult, onTable }: { database: LeagueDatabase; standings: Standing[]; scorers: PlayerStat[]; nextMatch?: Match; confirmedCount: number; pendingCount: number; liveMatchday: number; onFixtures: () => void; onResult: () => void; onTable: () => void }) {
  const event = nextMatch || database.matches.filter((match) => match.status === "CONFIRMED").sort((a, b) => b.goals.length - a.goals.length)[0];
  const home = event ? teamById(database.teams, event.homeTeamId) : undefined;
  const away = event ? teamById(database.teams, event.awayTeamId) : undefined;
  const homeStanding = home ? standings.find((team) => team.id === home.id) : undefined;
  const awayStanding = away ? standings.find((team) => team.id === away.id) : undefined;
  const topScorer = scorers[0];
  const crisis = standings.at(-1);
  const homeRate = homeStanding?.played ? (homeStanding.points / homeStanding.played).toFixed(1) : "—";
  const awayRate = awayStanding?.played ? (awayStanding.points / awayStanding.played).toFixed(1) : "—";
  return <section className="broadcast-hub">
    <div className="broadcast-hub-bar"><div><p className="eyebrow">BROADCAST CONTROL / MATCHDAY {String(liveMatchday || 1).padStart(2, "0")}</p><h2>THE LEAGUE, IN MOTION.</h2></div><div className="broadcast-status"><span /><strong>LIVE DATABASE</strong><small>{confirmedCount} official results · {pendingCount} awaiting review</small></div></div>
    <div className="broadcast-main-grid">
      <article className="broadcast-event-card"><div className="broadcast-event-top"><span className="broadcast-kicker"><span className="broadcast-pulse" /> {nextMatch ? "MATCH OF THE ROUND" : "LATEST SIGNAL"}</span><span>{event ? `MATCHDAY ${event.matchday}` : "NO FIXTURE"}</span></div>{event ? <><div className="broadcast-event-meta"><span>{formatMatchDate(event.date)}</span><span>{event.status === "CONFIRMED" ? "FULL TIME" : "NEXT KICKOFF"}</span></div><h3>{home?.name || "Home side"}<em>vs</em>{away?.name || "Away side"}</h3><div className="broadcast-radar"><div><small>POINTS / GAME</small><strong>{homeRate}</strong><span>{homeStanding?.goalDifference && homeStanding.goalDifference > 0 ? `+${homeStanding.goalDifference}` : homeStanding?.goalDifference ?? 0} GD</span></div><div className="broadcast-radar-center"><span>H2H<br />SIGNAL</span><i /></div><div><small>POINTS / GAME</small><strong>{awayRate}</strong><span>{awayStanding?.goalDifference && awayStanding.goalDifference > 0 ? `+${awayStanding.goalDifference}` : awayStanding?.goalDifference ?? 0} GD</span></div></div><div className="broadcast-event-actions"><button onClick={onFixtures}>Open match centre <ArrowRight size={14} /></button>{nextMatch && <button className="broadcast-primary" onClick={onResult}><Plus size={14} /> Enter result</button>}</div></> : <div className="broadcast-empty">The league is waiting for its first scheduled fixture.</div>}</article>
      <aside className="broadcast-form-card"><div className="broadcast-card-heading"><div><p className="eyebrow">LIVE FORM & TITLE RACE</p><h3>Momentum, not marketing.</h3></div><Activity size={17} /></div>{standings.slice(0, 4).map((team, index) => <div className="broadcast-form-row" key={team.id}><span className="broadcast-rank">0{index + 1}</span><TeamMark team={team} size="sm" /><div><strong>{team.name}</strong><small>{team.points} pts · {team.won}W {team.drawn}D {team.lost}L</small></div><MiniSparkline values={team.form.map((result, formIndex) => formIndex + (result === "W" ? 3 : result === "D" ? 1 : 0))} tone={["#a7f06e", "#6eb7f0", "#f0b35b", "#d0c0fa"][index]} /></div>)}<button className="broadcast-link" onClick={onTable}>Open full standings <ArrowRight size={14} /></button></aside>
    </div>
    <div className="broadcast-intelligence-grid"><article className="broadcast-intelligence-card matrix-card"><div className="broadcast-card-heading"><div><p className="eyebrow">LIVE LEAGUE MATRIX</p><h3>Form, gap, pressure.</h3></div><BarChart3 size={17} /></div><div className="broadcast-matrix-list">{standings.slice(0, 5).map((team, index) => <div key={team.id}><b>{index + 1}</b><span>{team.shortName}</span><div className="matrix-track"><i style={{ width: `${Math.min(100, Math.max(10, team.points * 7))}%` }} /></div><strong>{team.points}</strong></div>)}</div></article><article className="broadcast-intelligence-card boot-card"><div className="broadcast-card-heading"><div><p className="eyebrow">GOLDEN BOOT RACE</p><h3>{topScorer ? topScorer.name : "Awaiting scorers"}</h3></div><Trophy size={17} /></div>{topScorer ? <><div className="boot-score"><strong>{topScorer.goals}</strong><span>official goals</span></div><p>{topScorer.teamName} leads the confirmed goal ledger.</p><div className="boot-list">{scorers.slice(0, 3).map((player) => <span key={`${player.teamId}-${player.name}`}><b>{player.name}</b><em>{player.goals}</em></span>)}</div></> : <p className="muted-copy">Golden Boot data appears after confirmed goals.</p>}</article><article className="broadcast-intelligence-card crisis-card"><div className="broadcast-card-heading"><div><p className="eyebrow">CRISIS RADAR</p><h3>{crisis ? crisis.name : "No crisis"}</h3></div><AlertTriangle size={17} /></div>{crisis ? <><div className="crisis-pressure"><strong>{crisis.points}</strong><span>points<br />under pressure</span></div><div className="crisis-bar"><i style={{ width: `${Math.max(12, Math.min(100, 100 - crisis.points * 8 + Math.max(0, -crisis.goalDifference) * 3))}%` }} /></div><p>{crisis.name} sits at the bottom of the live table on {crisis.goalDifference > 0 ? "+" : ""}{crisis.goalDifference} GD.</p></> : <p className="muted-copy">The crisis radar will activate once the table has data.</p>}</article></div>
    <div className="broadcast-footnote"><span><span className="broadcast-pulse" /> Source of truth: confirmed results only</span><span>Top threat: {topScorer ? `${topScorer.name} · ${topScorer.goals} goals` : "no official scorer yet"}</span></div>
  </section>;
}

function TeamDossierDrawer({ database, team, standing, onClose }: { database: LeagueDatabase; team: Team; standing?: Standing; onClose: () => void }) {
  const playerGoals = database.matches.filter((match) => match.status === "CONFIRMED").flatMap((match) => match.goals).filter((goal) => goal.teamId === team.id).reduce<Record<string, number>>((result, goal) => { result[goal.playerName] = (result[goal.playerName] || 0) + 1; return result; }, {});
  const topPlayers = Object.entries(playerGoals).sort((a, b) => b[1] - a[1]).slice(0, 5);
  const nextOpponents = database.matches.filter((match) => match.status === "SCHEDULED" && (match.homeTeamId === team.id || match.awayTeamId === team.id)).slice(0, 3);
  const wins = standing?.won || 0;
  const winRate = standing?.played ? Math.round((wins / standing.played) * 100) : 0;
  const biggestWin = database.matches.filter((match) => match.status === "CONFIRMED" && (match.homeTeamId === team.id || match.awayTeamId === team.id) && match.homeScore !== null && match.awayScore !== null).sort((a, b) => Math.abs((b.homeScore || 0) - (b.awayScore || 0)) - Math.abs((a.homeScore || 0) - (a.awayScore || 0)))[0];
  return <div className="drawer-backdrop" onClick={onClose}><aside className="team-dossier-drawer" onClick={(event) => event.stopPropagation()}><button className="drawer-close" onClick={onClose} aria-label="Close team dossier"><X size={18} /></button><div className="dossier-identity"><TeamMark team={team} size="lg" /><div><p className="eyebrow">CLUB COMMAND CENTER</p><h2>{team.name}</h2><p>{team.manager} · {team.shortName}</p></div></div><div className="dossier-record"><span><b>{standing?.points || 0}</b><small>points</small></span><span><b>{winRate}%</b><small>win rate</small></span><span><b>{standing?.goalDifference || 0}</b><small>goal diff</small></span></div><div className="dossier-section"><p className="eyebrow">DANGER MEN</p>{topPlayers.length ? <div className="dossier-player-list">{topPlayers.map(([name, goals]) => <div key={name}><span className="dossier-player-avatar">{initials(name)}</span><strong>{name}</strong><b>{goals}<small> goals</small></b></div>)}</div> : <p className="muted-copy">No confirmed scorers recorded for this club yet.</p>}</div><div className="dossier-section"><p className="eyebrow">MANAGER RECORD</p><div className="dossier-audit"><span><b>{standing?.played || 0}</b><small>matches audited</small></span><span><b>{biggestWin ? `${biggestWin.homeScore}–${biggestWin.awayScore}` : "—"}</b><small>biggest margin</small></span><span><b>{standing?.form.length || 0}</b><small>form signals</small></span></div></div><div className="dossier-section"><p className="eyebrow">NEXT THREE OPPONENTS</p>{nextOpponents.length ? <div className="dossier-opponents">{nextOpponents.map((match) => { const opponent = teamById(database.teams, match.homeTeamId === team.id ? match.awayTeamId : match.homeTeamId); return <div key={match.id}><span>MD{match.matchday}</span><strong>{opponent?.name || "Unknown opponent"}</strong><small>{formatMatchDate(match.date)}</small></div>; })}</div> : <p className="muted-copy">No upcoming opponents are scheduled.</p>}</div><div className="dossier-stamp"><ShieldCheck size={16} /><span>Verified club record<br /><small>Based on confirmed results</small></span></div></aside></div>;
}

function MatchDossierDrawer({ database, match, onClose }: { database: LeagueDatabase; match: Match; onClose: () => void }) {
  const home = teamById(database.teams, match.homeTeamId);
  const away = teamById(database.teams, match.awayTeamId);
  const previous = database.matches.filter((item) => item.id !== match.id && item.status === "CONFIRMED" && ((item.homeTeamId === match.homeTeamId && item.awayTeamId === match.awayTeamId) || (item.homeTeamId === match.awayTeamId && item.awayTeamId === match.homeTeamId))).slice(-3);
  const goals = match.goals.slice().sort((a, b) => a.minute - b.minute);
  return <div className="drawer-backdrop" onClick={onClose}><aside className="match-dossier-drawer" onClick={(event) => event.stopPropagation()}><button className="drawer-close" onClick={onClose} aria-label="Close match dossier"><X size={18} /></button><p className="eyebrow">MATCH DOSSIER · MATCHDAY {match.matchday}</p><h2>{home?.name || "Home"} <em>vs</em> {away?.name || "Away"}</h2><div className="dossier-match-meta"><span>{formatMatchDate(match.date)}</span><span className={`status-chip ${statusClass(match.status)}`}><span />{statusLabel(match.status)}</span></div><div className="dossier-score"><TeamMark team={home} size="lg" /><strong>{match.homeScore === null ? "—" : match.homeScore}</strong><i>–</i><strong>{match.awayScore === null ? "—" : match.awayScore}</strong><TeamMark team={away} size="lg" /></div><div className="dossier-section"><p className="eyebrow">90-MINUTE TIMELINE</p>{goals.length ? <div className="dossier-timeline"><div className="dossier-timeline-line" />{goals.map((goal, index) => <div className="dossier-timeline-event" key={`${goal.id}-${index}`} style={{ left: `${Math.max(4, Math.min(94, goal.minute))}%` }}><b>{goal.minute}′</b><span>{goal.playerName}</span><small>{teamById(database.teams, goal.teamId)?.shortName || "TEAM"}</small></div>)}</div> : <p className="muted-copy">Goal pings will appear here when the home team submits a result.</p>}</div><div className="dossier-section"><p className="eyebrow">HEAD-TO-HEAD FILE</p>{previous.length ? previous.map((item) => <div className="dossier-h2h-row" key={item.id}><span>MD{item.matchday}</span><strong>{item.homeScore}–{item.awayScore}</strong><small>{formatMatchDate(item.date)}</small></div>) : <p className="muted-copy">No previous confirmed meeting in this league.</p>}</div></aside></div>;
}
...
function StandingsMatrix({ standings }: { standings: Standing[] }) {
  const max = Math.max(1, ...standings.map((team) => team.points));
  return <div className="standings-matrix">{standings.map((team, index) => <div className="matrix-row" key={team.id}><span className="matrix-rank">{String(index + 1).padStart(2, "0")}</span><TeamMark team={team} size="sm" /><strong>{team.shortName}</strong><div className="matrix-bar"><i style={{ width: `${Math.max(4, (team.points / max) * 100)}%`, background: team.accent }} /></div><b>{team.points}</b><small>{team.goalDifference > 0 ? "+" : ""}{team.goalDifference} GD</small></div>)}</div>;
}

function AwardsBoard({ playerStats, teamPerformance, activeTab, onTab }: { playerStats: PlayerStat[]; teamPerformance: TeamPerformance[]; activeTab: "attack" | "defense"; onTab: (tab: "attack" | "defense") => void }) {
  const attacks = [...playerStats].sort((a, b) => b.officialGoals - a.officialGoals || b.appearances - a.appearances).slice(0, 3);
  const defenses = [...teamPerformance].sort((a, b) => b.cleanSheets - a.cleanSheets || a.goalsAgainst - b.goalsAgainst).slice(0, 3);
  return <section className="awards-board"><div className="panel-header"><div><p className="eyebrow">PODIUM / LIVE AWARDS</p><h2>{activeTab === "attack" ? "Golden Boot" : "The Dutch Wall"}</h2></div><Trophy size={18} className="panel-icon" /></div><div className="award-tabs"><button type="button" className={activeTab === "attack" ? "selected" : ""} onClick={() => onTab("attack")}>Attack</button><button type="button" className={activeTab === "defense" ? "selected" : ""} onClick={() => onTab("defense")}>Defense</button></div>{activeTab === "attack" ? <div className="podium-list">{attacks.map((player, index) => <div className={`podium-row podium-${index + 1}`} key={`${player.teamId}-${player.name}`}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{player.name}</strong><small>{player.teamName}</small></div><b>{player.officialGoals}<small> goals</small></b></div>)}{!attacks.length && <p className="muted-copy">Confirmed goals unlock the podium.</p>}</div> : <div className="podium-list">{defenses.map((team, index) => <div className={`podium-row podium-${index + 1}`} key={team.teamId}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{team.teamName}</strong><small>{team.goalsAgainst} conceded · {team.matchesPlayed} played</small></div><b>{team.cleanSheets}<small> CS</small></b></div>)}{!defenses.length && <p className="muted-copy">Confirmed results unlock defensive records.</p>}</div>}</section>;
}

function WhatIfPanel({ database, teams, scores, onScoreChange }: { database: LeagueDatabase; teams: Team[]; scores: Record<string, { home: string; away: string }>; onScoreChange: (matchId: string, side: "home" | "away", value: string) => void }) {
  const open = database.matches.filter((match) => match.status !== "CONFIRMED").slice(0, 6);
  return <section className="what-if-panel"><div className="panel-header"><div><p className="eyebrow">SCENARIO LAB / NON-OFFICIAL</p><h2>What if the next fixtures land?</h2><p className="section-caption">Model outcomes without touching the official database.</p></div><Gauge size={18} className="panel-icon" /></div><div className="what-if-list">{open.map((match) => { const home = teamById(teams, match.homeTeamId); const away = teamById(teams, match.awayTeamId); const values = scores[match.id] || { home: "", away: "" }; return <div className="what-if-row" key={match.id}><span>MD{match.matchday}</span><strong>{home?.shortName || "HOME"}</strong><input aria-label={`${home?.name || "Home"} projected score`} inputMode="numeric" value={values.home} onChange={(event) => onScoreChange(match.id, "home", event.target.value.replace(/\D/g, "").slice(0, 2))} /><em>—</em><input aria-label={`${away?.name || "Away"} projected score`} inputMode="numeric" value={values.away} onChange={(event) => onScoreChange(match.id, "away", event.target.value.replace(/\D/g, "").slice(0, 2))} /><strong>{away?.shortName || "AWAY"}</strong></div>; })}{!open.length && <p className="muted-copy">All scheduled fixtures are already official.</p>}</div></section>;
}

function AnalyticsWorkspace({ standings, performance, playerStats }: { standings: Standing[]; performance: TeamPerformance[]; playerStats: PlayerStat[] }) {
  const maxGoals = Math.max(1, ...performance.map((team) => team.goalsFor));
  const maxAgainst = Math.max(1, ...performance.map((team) => team.goalsAgainst));
  const topDefense = [...performance].sort((a, b) => a.goalsAgainst - b.goalsAgainst || b.cleanSheets - a.cleanSheets || a.teamName.localeCompare(b.teamName)).slice(0, 5);
  const dependency = standings.map((standing) => {
    const teamPlayers = playerStats.filter((player) => player.teamId === standing.id);
    const topPlayer = [...teamPlayers].sort((a, b) => b.officialGoals - a.officialGoals)[0];
    const teamGoals = performance.find((team) => team.teamId === standing.id)?.goalsFor || 0;
    return { team: standing, topPlayer, teamGoals, share: teamGoals && topPlayer ? Math.round((topPlayer.officialGoals / teamGoals) * 100) : 0 };
  }).sort((a, b) => b.share - a.share).slice(0, 6);
  const topScorers = [...playerStats].sort((a, b) => b.officialGoals - a.officialGoals || b.appearances - a.appearances).slice(0, 3);
  return <section className="analytics-workspace">
    <div className="analytics-hero"><div><p className="eyebrow">ANALYTICS / CONFIRMED DATA ONLY</p><h2>Read the shape of the league.</h2><p>Scatter, dependency, and defensive pressure views built from official match results. No projections are mixed into this desk.</p></div><div className="analytics-hero-stats"><span><strong>{standings.length}</strong><small>clubs tracked</small></span><span><strong>{playerStats.length}</strong><small>players indexed</small></span></div></div>
    <div className="analytics-main-grid">
      <section className="analytics-panel analytics-scatter-panel"><div className="panel-header"><div><p className="eyebrow">01 / PERFORMANCE MAP</p><h2>Attack × pressure</h2><p className="section-caption">Goals for against goals conceded. Lower-right is a warning zone.</p></div><Activity size={18} className="panel-icon" /></div>{performance.length ? <div className="analytics-scatter-wrap"><svg viewBox="0 0 640 310" role="img" aria-label="Attack versus goals conceded scatter plot"><defs><linearGradient id="scatterGlow" x1="0" x2="1"><stop offset="0" stopColor="#6eb7f0" /><stop offset="1" stopColor="#a7f06e" /></linearGradient></defs><rect x="48" y="18" width="566" height="238" fill="rgba(255,255,255,.015)" stroke="rgba(219,239,231,.12)" /><line x1="48" y1="256" x2="614" y2="256" stroke="rgba(219,239,231,.18)" /><line x1="48" y1="18" x2="48" y2="256" stroke="rgba(219,239,231,.18)" />{performance.map((team) => { const x = 48 + (team.goalsFor / maxGoals) * 566; const y = 256 - (team.goalsAgainst / maxAgainst) * 238; return <g key={team.teamId}><circle cx={x} cy={y} r="7" fill={team.accent || "#a7f06e"} stroke="#0b1115" strokeWidth="3"><title>{team.teamName}: {team.goalsFor} for, {team.goalsAgainst} against</title></circle><text x={x + 10} y={y + 4} fill="#b6c6c7" fontSize="10" fontFamily="Space Mono, monospace">{team.shortName}</text></g>; })}<text x="50" y="278" fill="#70858a" fontSize="9" fontFamily="Space Mono, monospace">0 GOALS FOR</text><text x="555" y="278" fill="#70858a" fontSize="9" fontFamily="Space Mono, monospace">{maxGoals} FOR</text><text x="12" y="27" fill="#70858a" fontSize="9" fontFamily="Space Mono, monospace">{maxAgainst} AGAINST</text><text x="14" y="253" fill="#70858a" fontSize="9" fontFamily="Space Mono, monospace">0 AGAINST</text><path d="M48 256 L614 18" stroke="url(#scatterGlow)" strokeDasharray="5 8" opacity=".55" /></svg></div> : <p className="muted-copy">Confirmed matches will populate the map.</p>}</section>
      <section className="analytics-panel"><div className="panel-header"><div><p className="eyebrow">02 / DEFENSIVE GRID</p><h2>Who bends first?</h2></div><ShieldCheck size={18} className="panel-icon" /></div><div className="analytics-rank-list">{topDefense.map((team, index) => <div className="analytics-rank-row" key={team.teamId}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{team.teamName}</strong><small>{team.cleanSheets} clean sheets · {team.matchesPlayed} played</small><i><b style={{ width: `${Math.max(4, 100 - Math.min(92, (team.goalsAgainst / Math.max(1, maxAgainst)) * 100))}%` }} /></i></div><strong>{team.goalsAgainst}</strong></div>)}{!topDefense.length && <p className="muted-copy">No defensive sample yet.</p>}</div></section>
    </div>
    <div className="analytics-bottom-grid">
      <section className="analytics-panel"><div className="panel-header"><div><p className="eyebrow">03 / SCORER DEPENDENCY</p><h2>One-player gravity</h2><p className="section-caption">Share of each club’s confirmed goals from its leading scorer.</p></div><Target size={18} className="panel-icon" /></div><div className="dependency-list">{dependency.map((row) => <div className="dependency-row" key={row.team.id}><TeamMark team={row.team} size="sm" /><div><strong>{row.team.shortName}</strong><small>{row.topPlayer ? `${row.topPlayer.name} · ${row.topPlayer.officialGoals} goals` : "No official scorer"}</small></div><div className="dependency-track"><i style={{ width: `${Math.min(100, row.share)}%` }} /></div><b>{row.share ? `${row.share}%` : "—"}</b></div>)}{!dependency.length && <p className="muted-copy">Scorer dependency appears after confirmed results.</p>}</div></section>
      <section className="analytics-panel analytics-podium-panel"><div className="panel-header"><div><p className="eyebrow">04 / PODIUM SIGNAL</p><h2>Golden Boot radar</h2></div><Trophy size={18} className="panel-icon" /></div><div className="analytics-podium">{topScorers.map((player, index) => <div className={`analytics-podium-row podium-${index + 1}`} key={`${player.teamId}-${player.name}`}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{player.name}</strong><small>{player.teamName}</small></div><b>{player.officialGoals}</b></div>)}{!topScorers.length && <p className="muted-copy">Confirmed goals unlock the podium.</p>}</div></section>
    </div>
  </section>;
}
...
function LeagueSignalsPanel({ database, performance, playerStats }: { database: LeagueDatabase; performance: TeamPerformance[]; playerStats: PlayerStat[] }) {
  const boot = [...playerStats].sort((a, b) => b.officialGoals - a.officialGoals || b.appearances - a.appearances).slice(0, 3);
  const glove = [...performance].sort((a, b) => b.cleanSheets - a.cleanSheets || a.goalsAgainst - b.goalsAgainst).slice(0, 3);
  const clubs = database.teams.slice(0, 4);
  const headToHead = (homeId: string, awayId: string) => database.matches.filter((match) => match.status === "CONFIRMED" && ((match.homeTeamId === homeId && match.awayTeamId === awayId) || (match.homeTeamId === awayId && match.awayTeamId === homeId))).map((match) => `${match.homeScore}-${match.awayScore}`).join(" · ") || "—";
  return <section className="panel league-signals-panel"><div className="panel-header"><div><p className="eyebrow">LIVE LEAGUE SIGNALS</p><h2>Records, not stock photos.</h2><p className="section-caption">The visual layer now follows the confirmed ledger: attack, clean sheets, and actual club meetings.</p></div><BarChart3 size={18} className="panel-icon" /></div><div className="league-signals-grid"><article className="league-signal-card boot-signal"><div className="signal-card-heading"><span>GOLDEN BOOT</span><Trophy size={16} /></div>{boot.length ? boot.map((player, index) => <div className="signal-row" key={`${player.teamId}-${player.name}`}><span className="signal-rank">0{index + 1}</span><div><strong>{player.name}</strong><small>{player.teamName}</small></div><b>{player.officialGoals}<small> G</small></b></div>) : <p className="muted-copy">Confirmed goals unlock the scoring race.</p>}</article><article className="league-signal-card glove-signal"><div className="signal-card-heading"><span>GOLDEN GLOVE</span><ShieldCheck size={16} /></div>{glove.length ? glove.map((team, index) => <div className="signal-row" key={team.teamId}><span className="signal-rank">0{index + 1}</span><div><strong>{team.teamName}</strong><small>{team.goalsAgainst} conceded · {team.matchesPlayed} played</small></div><b>{team.cleanSheets}<small> CS</small></b></div>) : <p className="muted-copy">Confirmed results unlock clean-sheet records.</p>}</article></div><div className="h2h-widget"><div className="signal-card-heading"><span>HEAD-TO-HEAD MATRIX · TOP FOUR CLUBS</span><span className="h2h-note">CONFIRMED ONLY</span></div><div className="h2h-grid" style={{ gridTemplateColumns: `repeat(${clubs.length + 1}, minmax(52px, 1fr))` }}><span className="h2h-corner" />{clubs.map((team) => <span className="h2h-axis" key={`top-${team.id}`} style={{ color: team.accent }}>{team.shortName}</span>)}{clubs.map((rowTeam) => <><span className="h2h-axis h2h-row-axis" key={`side-${rowTeam.id}`} style={{ color: rowTeam.accent }}>{rowTeam.shortName}</span>{clubs.map((columnTeam) => <span className={`h2h-cell ${rowTeam.id === columnTeam.id ? "self" : ""}`} key={`${rowTeam.id}-${columnTeam.id}`}>{rowTeam.id === columnTeam.id ? "·" : headToHead(rowTeam.id, columnTeam.id)}</span>)}</>)}</div></div></section>;
}
...
export default function Home() {
  ...
  const [selectedFixtureMatchday, setSelectedFixtureMatchday] = useState<number | null>(null);
  const [fixtureTeamFilter, setFixtureTeamFilter] = useState("all");
  const [standingsMode, setStandingsMode] = useState<"table" | "matrix">("table");
  const [standingsAwardsTab, setStandingsAwardsTab] = useState<"attack" | "defense">("attack");
  const [whatIfEnabled, setWhatIfEnabled] = useState(false);
  const [whatIfScores, setWhatIfScores] = useState<Record<string, { home: string; away: string }>>({});
  const [teamDirectoryFilter, setTeamDirectoryFilter] = useState<"all" | "contenders" | "crisis">("all");
  const [teamDirectorySort, setTeamDirectorySort] = useState<"points" | "attack" | "cleanSheets">("points");
  ...
  const filteredMatches = useMemo(() => scopedMatches.filter((match) => {
    ...
    const teamMatch = fixtureTeamFilter === "all" || match.homeTeamId === fixtureTeamFilter || match.awayTeamId === fixtureTeamFilter;
    const matchdayMatch = !effectiveFixtureMatchday || Number(match.matchday) === Number(effectiveFixtureMatchday);
    return statusMatch && teamMatch && matchdayMatch && (!searchTerm || home.includes(searchTerm.toLowerCase()) || away.includes(searchTerm.toLowerCase()));
  ...
  const displayedStandings = useMemo(() => whatIfEnabled ? projectedStandings(database, standings, whatIfScores) : standings, [database, standings, whatIfEnabled, whatIfScores]);
  const teamDirectoryRows = useMemo(() => {
    const rows = database.teams.map((team) => {
      const standing = standings.find((item) => item.id === team.id);
      const performance = teamPerformance.find((item) => item.teamId === team.id);
      const danger = playerStats.filter((player) => player.teamId === team.id).sort((a, b) => b.officialGoals - a.officialGoals || b.totalGoals - a.totalGoals)[0];
      const next = database.matches.find((match) => match.status === "SCHEDULED" && (match.homeTeamId === team.id || match.awayTeamId === team.id));
      const opponent = next ? teamById(database.teams, next.homeTeamId === team.id ? next.awayTeamId : next.homeTeamId) : undefined;
      const pressure = standing?.played ? Math.round(Math.min(99, Math.max(4, (standing.lost * 18) + Math.max(0, -standing.goalDifference) * 4 + (standing.points === 0 ? 22 : 0)))) : 0;
      return { team, standing, performance, danger, next, opponent, pressure };
    });
    return rows.filter((row) => teamDirectoryFilter === "all" || (teamDirectoryFilter === "contenders" ? (row.standing?.points || 0) >= Math.max(1, (standings[0]?.points || 0) - 3) : row.pressure >= 45)).sort((a, b) => teamDirectorySort === "attack" ? (b.performance?.goalsFor || 0) - (a.performance?.goalsFor || 0) : teamDirectorySort === "cleanSheets" ? (b.performance?.cleanSheets || 0) - (a.performance?.cleanSheets || 0) : (b.standing?.points || 0) - (a.standing?.points || 0));
  }, [database.matches, database.teams, playerStats, standings, teamDirectoryFilter, teamDirectorySort, teamPerformance]);
  ...
  const currentTitle = activeView === "overview" ? (userTeam?.name ? `${userTeam.name} matchday desk` : isAdmin ? "League command centre" : "League matchday desk") : activeView === "fixtures" ? "Fixtures & results" : activeView === "teams" ? "Teams & managers" : activeView === "table" ? "Live league table" : activeView === "analytics" ? "League analytics" : activeView === "chronicle" ? "The Khalpar Chronicle" : activeView === "database" ? "Database & access" : "Rules & tie-breakers";
  const currentDescription = activeView === "overview" ? (userTeam?.name ? `Your ${userTeam.name} view for matchday ${liveMatchday || "the current round"}: fixtures, results, and team progress in one place.` : "Keep the league moving with one clear place for every result, roster, and ranking.") : activeView === "fixtures" ? "Every scheduled match, submission, and confirmation in one calm workflow." : activeView === "teams" ? "Manage the clubs and eFootball accounts that power this season." : activeView === "table" ? "Official standings calculated from confirmed match results." : activeView === "analytics" ? "Attack, defense, dependency, and podium signals from confirmed match data." : activeView === "chronicle" ? "The pulse of League'de Khalpar: signed editorials, verified facts, and the evidence behind the story." : activeView === "database" ? "Manage identity, role permissions, and the records behind this competition." : "Automated rules keep tied teams ordered without manual spreadsheet work.";
    ...
    <div className={`league-app ${mobileNavOpen ? "mobile-nav-open" : ""}`}>
      <aside className="league-sidebar">
        ...
        <nav className="league-nav" aria-label="League navigation">
          ...
          <button className={activeView === "analytics" ? "active" : ""} onClick={() => { setActiveView("analytics"); setMobileNavOpen(false); }}><BarChart3 size={17} />Analytics</button>
          <button className={activeView === "teams" ? "active" : ""} onClick={() => { setActiveView("teams"); setMobileNavOpen(false); }}><Users size={17} />Teams</button>
          <button className={activeView === "chronicle" ? "active" : ""} onClick={() => { setActiveView("chronicle"); setMobileNavOpen(false); }}><Newspaper size={17} />Chronicle<span className="nav-shortcut">06</span></button>
          <p className="nav-label nav-label-spaced">League tools</p>
          {isAdmin && <>
            <button className={activeView === "database" ? "active" : ""} onClick={() => { setActiveView("database"); setMobileNavOpen(false); }}><Database size={17} />Database<span className="nav-live">LIVE</span></button>
      ...
      <div className="league-main">
        ...
        <main className="league-content">
          ...
          {activeView === "overview" && <>
            <BroadcastHub database={database} standings={standings} scorers={scorers} nextMatch={nextMatch} confirmedCount={confirmedCount} pendingCount={pendingCount} liveMatchday={liveMatchday} onFixtures={() => setActiveView("fixtures")} onResult={() => openResult()} onTable={() => setActiveView("table")} />
            <section className="panel overview-race-panel"><div className="panel-header"><div><p className="eyebrow">TITLE RACE / CONFIRMED POINTS</p><h2>Who is pulling away?</h2><p className="section-caption">A matchday-by-matchday readout built only from official results.</p></div><Activity size={18} className="panel-icon" /></div><TitleRaceChart database={database} standings={standings} /></section>
            ...
            <LeagueSignalsPanel database={database} performance={teamPerformance} playerStats={playerStats} />
            <section className="panel next-fixtures-panel"><div className="panel-header"><div><p className="eyebrow">03 / NEXT UP</p><h2>Fixture desk</h2></div><button className="panel-link" onClick={() => setActiveView("fixtures")}>See all {database.matches.length} fixtures <ArrowRight size={14} /></button></div><div className="next-fixtures-list">{database.matches.filter((match) => match.status !== "CONFIRMED").slice(0, 3).map((match) => <MatchRow key={match.id} database={database} match={match} user={user} isAdmin={isAdmin} onResult={openResult} onConfirm={confirmMatch} onReschedule={rescheduleMatch} onSelect={setSelectedMatchId} />)}</div></section>
          </>}
          ...
          {activeView === "fixtures" && <section className="view-panel fixture-desk"><div className="fixture-desk-hero"><div><p className="eyebrow">THE MATCH CENTRE</p><h2>Fixtures, results & matchday rhythm</h2><p>See every league fixture at a glance. Your team’s matches are highlighted; every other result stays visible for the whole competition.</p></div><div className="fixture-desk-stats"><span><strong>{database.matches.length}</strong><small>total fixtures</small></span><span><strong>{confirmedCount}</strong><small>official results</small></span><span><strong>{ownedMatches.length}</strong><small>your fixtures</small></span></div></div><div className="fixture-legend"><span className="legend-item legend-owned"><i /> Your fixtures</span><span className="legend-item"><i /> Other league fixtures</span><span className="legend-note">Pending scores remain provisional until admin confirmation.</span></div><div className="fixture-layout"><div className="fixture-main"><FixtureMatchdayStepper database={database} activeMatchday={effectiveFixtureMatchday || liveMatchday} liveMatchday={liveMatchday} onChange={(matchday) => setSelectedFixtureMatchday(matchday)} /><div className="fixture-toolbar"><div className="fixture-toolbar-left"><div className="fixture-scope-tabs" role="tablist" aria-label="Fixture visibility"><button type="button" className={fixtureScope === "mine" ? "selected" : ""} onClick={() => setFixtureScope("mine")} disabled={!user.teamId} aria-pressed={fixtureScope === "mine"}>My fixtures <span>{ownedMatches.length}</span></button><button type="button" className={fixtureScope === "all" ? "selected" : ""} onClick={() => setFixtureScope("all")} aria-pressed={fixtureScope === "all"}>All fixtures <span>{database.matches.length}</span></button></div><div className="filter-tabs">{(["all", "upcoming", "pending", "completed"] as const).map((filter) => <button className={fixtureFilter === filter ? "selected" : ""} key={filter} onClick={() => setFixtureFilter(filter)}>{filter === "all" ? "All fixtures" : filter === "upcoming" ? "Upcoming" : filter === "pending" ? `Needs review ${pendingCount ? `· ${pendingCount}` : ""}` : "Completed"}</button>)}</div></div><label className="search-field"><Search size={15} /><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search teams" /></label></div><FixtureTeamFilter teams={database.teams} activeTeam={fixtureTeamFilter} onChange={setFixtureTeamFilter} /><div className="fixture-groups">{groupedFixtures.length ? groupedFixtures.map((group) => <section className="fixture-group" key={group.matchday}><div className="fixture-group-heading"><span className="matchday-number">{String(group.matchday).padStart(2, "0")}</span><div><p className="eyebrow">MATCHDAY {group.matchday}</p><h2>{formatMatchDate(group.matches[0].date)}</h2></div><span className="fixture-group-count">{group.matches.length} matches</span></div>{group.matches.map((match) => <MatchRow key={match.id} database={database} match={match} user={user} isAdmin={isAdmin} onResult={openResult} onConfirm={confirmMatch} onReschedule={rescheduleMatch} onSelect={setSelectedMatchId} />)}</section>) : <div className="empty-panel"><CalendarDays size={24} /><h3>No fixtures match this view.</h3><p>Try a different status or clear the team search.</p></div>}</div></div><aside className="today-fixtures-panel"><div className="today-panel-heading"><div><p className="eyebrow">LIVE TODAY</p><h3>Today’s fixtures</h3></div><CalendarDays size={18} /></div>{todayMatches.length ? <div className="today-fixtures-list">{todayMatches.map((match) => { const home = teamById(database.teams, match.homeTeamId); const away = teamById(database.teams, match.awayTeamId); const owned = isUserFixture(match, user); return <button type="button" className={`today-fixture ${owned ? "owned" : ""}`} key={match.id} onClick={() => setActiveView("fixtures")}><span className="today-fixture-time">{match.status === "CONFIRMED" ? "FT" : match.status === "PENDING" ? "PEN" : "—"}</span><span className="today-fixture-teams"><strong>{home?.shortName}</strong><span>{match.homeScore === null ? "vs" : `${match.homeScore} – ${match.awayScore}`}</span><strong>{away?.shortName}</strong></span><small>{owned ? "Your fixture" : `Matchday ${match.matchday}`}</small></button>; })}</div> : <div className="today-empty"><CalendarDays size={20} /><strong>No matches today</strong><span>The next matchday will appear here when it opens.</span></div>}<div className="today-panel-footer"><span><span className="today-live-dot" /> Live schedule</span><button type="button" onClick={() => setFixtureFilter("upcoming")}>View upcoming <ArrowRight size={13} /></button></div></aside></div></section>}

          {activeView === "teams" && <section className="view-panel"><div className="view-toolbar"><div><p className="eyebrow">ROSTER DIRECTORY</p><h2>{database.teams.length} teams in this season</h2><p className="section-caption">Only approved teams are included in generated fixtures.</p></div><div className="view-toolbar-actions">{isAdmin && <Button disabled={!useBackend || tournamentStarted || approvedTeamCount < 2} title={!useBackend ? "Live backend mode is required" : tournamentStarted ? "This tournament already has fixtures" : approvedTeamCount < 2 ? "Approve at least two teams first" : "Start the tournament and generate fixtures"} onClick={startTournament}><Flag size={16} /> {tournamentStarted ? "Tournament active" : "Start tournament"}</Button>}{isAdmin && useBackend && tournamentStarted && <Button variant="destructive" title="Delete the current fixtures and restart this tournament" onClick={resetTournament}><RefreshCw size={16} /> Reset tournament</Button>}{isAdmin ? <Button variant="outline" onClick={() => setShowTeamForm((current) => !current)}>{showTeamForm ? <X size={16} /> : <Plus size={16} />} {showTeamForm ? "Close form" : "Add team"}</Button> : <span className="read-only-note"><Eye size={14} /> Player view · read only</span>}</div></div>{isAdmin && database.teams.some((team) => team.approvalStatus === "PENDING") && <section className="approval-panel panel"><div className="panel-header"><div><p className="eyebrow">ADMIN QUEUE</p><h2>Teams awaiting approval</h2></div><Clock3 size={18} className="panel-icon" /></div><div className="approval-list">{database.teams.filter((team) => team.approvalStatus === "PENDING").map((team) => <div className="approval-row" key={team.id}><TeamMark team={team} size="sm" /><div><strong>{team.name}</strong><small>{team.manager} · {team.createdByEmail || "Registration request"}</small></div><div className="approval-actions"><Button onClick={() => decideTeam(team.id, "approve")}><Check size={14} /> Approve</Button><Button variant="outline" onClick={() => decideTeam(team.id, "reject")}><X size={14} /> Reject</Button></div></div>)}</div></section>}{showTeamForm && <AddTeamPanel onClose={() => setShowTeamForm(false)} onAdd={addTeam} />}{editingTeamId && (() => { const editTeam = database.teams.find((item) => item.id === editingTeamId); return editTeam ? <EditTeamPanel team={editTeam} onClose={() => setEditingTeamId(null)} onSave={(name, shortCode) => updateTeam(editingTeamId, name, shortCode)} /> : null; })()}<div className="club-directory-controls"><div className="club-filter-tabs" role="tablist" aria-label="Club scouting filters"><button type="button" className={teamDirectoryFilter === "all" ? "selected" : ""} onClick={() => setTeamDirectoryFilter("all")}>All clubs <span>{database.teams.length}</span></button><button type="button" className={teamDirectoryFilter === "contenders" ? "selected" : ""} onClick={() => setTeamDirectoryFilter("contenders")}>Title contenders</button><button type="button" className={teamDirectoryFilter === "crisis" ? "selected" : ""} onClick={() => setTeamDirectoryFilter("crisis")}>Crisis zone</button></div><label className="club-sort-control">SORT BY<select value={teamDirectorySort} onChange={(event) => setTeamDirectorySort(event.target.value as "points" | "attack" | "cleanSheets")}><option value="points">Points</option><option value="attack">Attack / GF</option><option value="cleanSheets">Clean sheets</option></select></label></div><div className="team-grid">{teamDirectoryRows.map(({ team, standing: row, performance, danger, next, opponent, pressure }, index) => { const approval = team.approvalStatus || "APPROVED"; return <article className="team-card franchise-card" key={team.id} style={{ "--team-accent": team.accent } as CSSProperties}><div className="team-card-top"><div className="club-crest-stage"><TeamMark team={team} size="lg" /></div><span className="team-card-rank">#{String(index + 1).padStart(2, "0")}</span>{isAdmin ? <div className="team-card-actions"><button className="icon-button" type="button" title={`Edit ${team.name}`} aria-label={`Edit ${team.name}`} onClick={() => setEditingTeamId(team.id)}><Pencil size={15} /></button><button className="icon-button danger-icon" type="button" title={`Delete ${team.name}`} aria-label={`Delete ${team.name}`} onClick={() => deleteTeam(team.id)}><Trash2 size={16} /></button></div> : <span className="team-card-rank-spacer" aria-hidden="true" />}</div><div className="franchise-card-heading"><div><h3>{team.name}</h3><p><UserRound size={13} /> {team.manager} · {team.shortName}</p></div><div className="franchise-form">{row?.form.slice(-4).map((value, formIndex) => <FormPill key={`${team.id}-${formIndex}`} value={value} />)}</div></div><div className="franchise-style-line"><span>TACTICAL DNA</span><strong>{danger ? `${danger.name} dependency` : "Awaiting scorer profile"}</strong><small>{next && opponent ? `Next: ${opponent.shortName} · MD${next.matchday}` : "No scheduled opponent"}</small></div><div className="danger-man-spotlight"><div className="dossier-player-avatar">{danger ? initials(danger.name) : "—"}</div><div><span className="eyebrow">DANGER MAN</span><strong>{danger?.name || "No confirmed scorer"}</strong><small>{danger ? `${danger.officialGoals} official goals · ${danger.appearances} apps` : "Confirmed results will populate this profile."}</small></div><span className="danger-rating" style={{ color: team.accent }}>{pressure ? `${Math.max(10, 100 - pressure)}` : "—"}<small>THREAT</small></span></div><div className="team-card-stats franchise-stats"><span><strong>{performance?.goalsFor ?? 0}</strong><small>GF</small></span><span><strong>{row?.goalsAgainst ?? 0}</strong><small>GA</small></span><span><strong>{performance?.cleanSheets ?? 0}</strong><small>clean sheets</small></span><span><strong>{row?.points ?? 0}</strong><small>points</small></span></div><div className="franchise-card-actions"><button type="button" className="team-card-footer" onClick={() => setSelectedTeamId(team.id)}><span className="team-record"><span className="record-dot" />Open club dossier</span><ArrowRight size={15} /></button>{next && <button type="button" className="scout-button" onClick={() => setSelectedMatchId(next.id)}><SlidersHorizontal size={14} /> Scout next</button>}</div></article>; })}</div></section>}

          {activeView === "table" && <section className="view-panel standings-desk"><div className="standings-desk-hero"><div><p className="eyebrow">LEAGUE INTELLIGENCE / LIVE</p><h2>Table, pressure, possibilities.</h2><p>Official results stay sacred. Explore form, attack, defense, and projected outcomes without changing the league record.</p></div><div className="standings-desk-actions"><div className="view-toggle" role="tablist" aria-label="Standings display"><button type="button" className={standingsMode === "table" ? "selected" : ""} onClick={() => setStandingsMode("table")}>Table</button><button type="button" className={standingsMode === "matrix" ? "selected" : ""} onClick={() => setStandingsMode("matrix")}>Matrix</button></div><button type="button" className={`scenario-toggle ${whatIfEnabled ? "active" : ""}`} onClick={() => setWhatIfEnabled((value) => !value)}><Gauge size={14} /> {whatIfEnabled ? "Scenario on" : "What-if lab"}</button></div></div><div className="table-view-grid"><section className="panel full-table-panel"><div className="panel-header"><div><p className="eyebrow">{whatIfEnabled ? "PROJECTED STANDINGS" : "OFFICIAL STANDINGS"}</p><h2>{database.league.name} table</h2></div><span className="table-updated"><span />{whatIfEnabled ? "Scenario only" : "Updated live"}</span></div>{standingsMode === "table" ? <StandingTable standings={displayedStandings} userTeamId={user?.teamId} /> : <StandingsMatrix standings={displayedStandings} />}</section><AwardsBoard playerStats={playerStats} teamPerformance={teamPerformance} activeTab={standingsAwardsTab} onTab={setStandingsAwardsTab} /></div>{whatIfEnabled && <WhatIfPanel database={database} teams={database.teams} scores={whatIfScores} onScoreChange={(matchId, side, value) => setWhatIfScores((current) => ({ ...current, [matchId]: { ...(current[matchId] || { home: "", away: "" }), [side]: value } }))} />}<PlayerStatsPanel stats={playerStats} /></section>}

          {activeView === "analytics" && <AnalyticsWorkspace standings={standings} performance={teamPerformance} playerStats={playerStats} />}
      ...
      {selectedTeam && <TeamDossierDrawer database={database} team={selectedTeam} standing={standings.find((row) => row.id === selectedTeam.id)} onClose={() => setSelectedTeamId(null)} />}
      {selectedMatch && <MatchDossierDrawer database={database} match={selectedMatch} onClose={() => setSelectedMatchId(null)} />}
    </div>
  );
}
