...
export default function Home() {
  ...
  async function sendNotifications() {
    ...
    try {
      ...
      toast.success("Matchday notifications sent", { description: `${result.sent} team card${result.sent === 1 ? "" : "s"} sent to the active player roster.` });
    } catch (error) {
  ...
  const currentTitle = activeView === "overview" ? (userTeam?.name ? `${userTeam.name} matchday desk` : isAdmin ? "League command centre" : "League matchday desk") : activeView === "fixtures" ? "Fixtures & results" : activeView === "teams" ? "Teams & managers" : activeView === "table" ? "Live league table" : activeView === "database" ? "Database & access" : "Rules & tie-breakers";
