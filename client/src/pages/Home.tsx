...
type View = "overview" | "fixtures" | "teams" | "table" | "chronicle" | "database" | "rules";
...
const CHRONICLE_TEMPLATE_EDITORIAL: ChronicleEditorial = {
  edition: "Matchday 5 Special",
  leadStory: { tag: "PLAYER IN FOCUS", headline: "DEL PIERO'S DECADE", subdeck: "10 goals in 4 matchdays: Soham FC's one-man empire.", statHighlight: { value: "10", label: "GOALS IN 4 MATCHDAYS" }, body: "Alessandro Del Piero is single-handedly carrying Soham FC to the top of the table with 10 goals in four matchdays.", accentColor: "#8B1E3F" },
  bentoHighlights: [
    { type: "BLOWOUT", tag: "CARNAGE", title: "Mbappé's Five-Star Rampage", detail: "Kylian Mbappé scored five goals across Real Tamo's 8–0 demolition of Barcelona.", scoreline: { home: "Real Tamo", homeScore: 8, away: "Barcelona", awayScore: 0, timeline: [{ player: "Mbappé", minute: "3'" }, { player: "Mbappé", minute: "32'" }, { player: "Mbappé", minute: "47'" }, { player: "Mbappé", minute: "55'" }, { player: "Mbappé", minute: "90'" }] }, accentColor: "#8B1E3F" },
    { type: "QUIRK", tag: "TACTICAL ANOMALY", title: "The Zlatan Paradox", detail: "Zlatan scored a hat-trick for Binod 11 and a first-minute strike for PrALaY FC in the same matchweek.", accentColor: "#C48A2A" },
    { type: "MEME", tag: "MANAGER ON PITCH", title: "Guardiola Boots Up", detail: "Pep Guardiola scored DXBJIT_'s opening goal in the 26th minute against Barcelona.", quote: "The manager found the net. The league found a new storyline.", accentColor: "#294C60" },
  ],
  crisisWatch: { team: "Chutpaglu", status: "FREE-FALL", stats: { played: 4, points: 0, gd: -14, goalsAgainstPerGame: 4.25 }, verdict: "Conceding 4.25 goals per game. Defensive system needs an immediate reset." },
};
...
function structuredEditorialFromForm(headline: string, dek: string, body: string, facts: string[], standings: Standing[], useTemplate: boolean): ChronicleEditorial {
  ...
  if (useTemplate) return { ...CHRONICLE_TEMPLATE_EDITORIAL, edition: `Matchday ${Math.max(1, standings.reduce((max, team) => Math.max(max, team.played), 0) + 1)} Special` };
...
function KhalparChroniclePanel({ editorials, standings, isAdmin, seasonId, busy, onPublish, onDelete }: { editorials: PunditEditorial[]; standings: Standing[]; isAdmin: boolean; seasonId: number | null; busy: boolean; onPublish: (input: { seasonId: number | null; publishDate: string; section: string; headline: string; dek: string; body: string; imageKey: string; facts: string[]; editorial?: ChronicleEditorial }) => void; onDelete: (id: string) => void }) {
  ...
  return <section className="panel chronicle-panel">
    ...
    {!hero ? <div className="chronicle-empty"><Newspaper size={22} /><div><strong>The Chronicle is waiting for its inaugural issue.</strong><p>Use the admin desk below to publish a signed editorial. The league data room remains separate and evidence-backed.</p></div></div> : <>
      <div className="chronicle-lead-grid"><article className="chronicle-hero-story"><div className="chronicle-image-wrap"><img src={chronicleImage(hero.imageKey)} alt="Editorial football moment" /><span>{lead?.tag || hero.section}</span>{lead?.statHighlight && <div className="chronicle-stat-chip"><strong>{lead.statHighlight.value}</strong><span>{lead.statHighlight.label}</span></div>}</div><div className="chronicle-hero-copy"><p className="chronicle-overline">{hero.publishDate} · {lead?.tag || "SPECIAL REPORT"}</p><h3>{lead?.headline || hero.headline}</h3><p className="chronicle-dek">{lead?.subdeck || hero.dek}</p><div className="chronicle-body chronicle-body-structured">{(lead?.body || hero.body).split(/\n{2,}/).slice(0, 2).map((paragraph, index) => <p className={index === 0 ? "drop-cap" : ""} key={`${hero.id}-paragraph-${index}`}>{paragraph}</p>)}</div><small>Filed by {hero.createdByEmail || "The Khalpar Chronicle desk"}</small>{isAdmin && <button className="chronicle-delete" onClick={() => onDelete(hero.id)} disabled={busy}><Trash2 size={13} /> Delete issue</button>}</div></article>
...
export default function Home() {
  ...
  const currentTitle = activeView === "overview" ? (userTeam?.name ? `${userTeam.name} matchday desk` : isAdmin ? "League command centre" : "League matchday desk") : activeView === "fixtures" ? "Fixtures & results" : activeView === "teams" ? "Teams & managers" : activeView === "table" ? "Live league table" : activeView === "chronicle" ? "The Khalpar Chronicle" : activeView === "database" ? "Database & access" : "Rules & tie-breakers";
  const currentDescription = activeView === "overview" ? (userTeam?.name ? `Your ${userTeam.name} view for matchday ${liveMatchday || "the current round"}: fixtures, results, and team progress in one place.` : "Keep the league moving with one clear place for every result, roster, and ranking.") : activeView === "fixtures" ? "Every scheduled match, submission, and confirmation in one calm workflow." : activeView === "teams" ? "Manage the clubs and eFootball accounts that power this season." : activeView === "table" ? "Official standings calculated from confirmed match results." : activeView === "chronicle" ? "The pulse of League'de Khalpar: signed editorials, verified facts, and the evidence behind the story." : activeView === "database" ? "Manage identity, role permissions, and the records behind this competition." : "Automated rules keep tied teams ordered without manual spreadsheet work.";
